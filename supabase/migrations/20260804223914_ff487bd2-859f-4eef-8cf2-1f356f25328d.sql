    ...
    loop
      ...
      then
        update public.execucao_etapas set lease_token = null, lease_ate = null,
          proxima_tentativa_em = null, ultimo_codigo_erro = 'dependencia_falhou' where id = r.eid;
        if r.est <> 'bloqueada' then
          perform public.aplicar_transicao_etapa(r.eid, 'bloqueada', 'dependencia falhou');
        end if;
        mudou := true;
      end if;
    end loop;
