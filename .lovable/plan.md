...
Saída do CTA — exatamente 5 itens, cada um com: `variacao_id`, `texto`, `tipo_acao`, `intensidade`, `intencao`, `formato_destino`, `alerta_promessa`, `alerta_cliche`, `justificativa`. Validação: 5 itens, ids únicos, sem campos extras, texto não vazio, enums fechados, sem duplicata literal, sem nota de auditoria inventada.
...
Saída da correção: exatamente um item corrigido por item solicitado, preservando tipo, formato, objetivo, vínculo com o original, papel de origem, versão do especialista, lote e `correcao_num = 1`. Bloqueios: id desconhecido, troca de tipo, correção sem original, duplicidade, item de outra execução, persistência parcial ou fora do schema.
...
- Cliente dispara os ramos em paralelo com `Promise.allSettled`; idempotência por etapa, lote e tentativa preservada; retomada após reload sem duplicar resultados.
...
- **concluida**: todos os formatos solicitados com ao menos um item aprovado e ranking completo.
- **parcialmente_concluida**: parte dos formatos ou parte dos lotes válida; resultados válidos preservados.
- **falhou**: nenhum resultado útil (ex.: CTA falha em pedido exclusivo de CTA).
- **aguardando_consentimento**: categoria exigida ausente na fotografia.
- **resultado_incerto**: chamada possivelmente processada sem confirmação; sem repetição automática, resolução manual explícita, item bloqueado no ranking.
- Falha de um especialista nunca apaga resultados dos outros; a correção só inicia para itens com auditoria válida e reprovada; nenhum item sem auditoria válida entra no ranking.
...
`orcamento_estimado` por papel (já existente) somado a: orçamento por etapa, teto por execução, reserva dedicada para uma correção e reserva para a segunda auditoria. Verificação de saldo antes de iniciar correção; esgotado, nenhuma chamada nova, resultados preservados, motivo informado, conclusão parcial quando houver conteúdo utilizável e nenhum fallback silencioso. Custo registrado por provedor, papel, etapa, lote, tentativa, correção e total da execução.
...
Migrações: categoria e termo `feedback_para_correcao`; grafo de dependências em `criar_execucao` com etapas `correcao` e `auditoria_final`; `reservar_etapas` com limite de concorrência; vínculo original↔correção em `execucao_resultados` com unicidade de uma correção por item; colunas de orçamento por etapa e reservas; `registry_validar` para CTA.
